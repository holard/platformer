...
public class MainChar extends Element {
	...
	public void move() {
		...
		if (lrud[2]) { // pressing the UP arrow
			...
			} else if (myMap.checkWall(x, y + 32)
					|| myMap.checkWall(x + SCALE - 1, y + 32)) {
				dy = -TIMER * 0.85;
			} else if (jumps == 1) {
				dy = -TIMER * 0.85;
				jumps -= 1;
		...
		if (!myMap.checkWall(x + SCALE, y + SCALE / 2)
				&& !myMap.checkWall(x - 1, y + SCALE / 2))
			stick = 0;
		x += dx;
		y += dy;
		if (x < 1) {
	...
	public void keyPressed(KeyEvent e) {
		...
		if (key == KeyEvent.VK_LEFT) {
			...
			lrud2[0] = true;
			setImage("craft2.png");
			if ((myMap.checkWall(x, y + SCALE) || myMap.checkWall(
					x + SCALE - 1, y + SCALE))
					&& lastkey == key
					&& keycount == 2) {
				dx = -0.9 * TIMER;
